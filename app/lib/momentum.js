...
/**
 * Compute a momentum score (0–100) and explainable breakdown.
 * Inputs are expected to come from CoinGecko /coins/markets with:
...
export function computeMomentumBreakdown(coin) {
  ...
  // Volatility proxy: large gap between 24h and 7d implies choppiness
  const volatilityProxy = Math.abs(c24 - c7);

  // Normalize contributions (guardrails against outliers)
...
/**
 * Confidence labels
 * High: score high + aligned + not too choppy
 * Medium: decent score + mild conflict/chop
 * Low: weak or noisy setup
 */
